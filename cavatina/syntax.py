...
def getTextAndRTFBoldRegion(rtf):
    ...
    for i in range(len(rtf)):
        if not fmt:
            # catch \\, ignoring the backslash escape sequence
            if rtf[i] == '\\' and (rtf[i + 1] != '\\' and rtf[i - 1] != '\\'):
    ...
    while i * 2 <= len(unpairedMatches) - 1:
        if i * 2 == len(unpairedMatches) - 1:
            ...
            boldRanges.append(
                (unpairedMatches[i * 2], unpairedMatches[i * 2 + 1]))
        i += 1
...
def tokenize(expr):
    ...
    for current in expr[1:]:
        ...
        if (  # new line
            ...
            (
                current == '\\' and (
                    previous in simple_punctuation or previous == ',\\')
            ) or (
            ...
            (
                ...
                (current == accidentals_symbols[
                 1] and previous[-1] == accidentals_symbols[1])
            )
        ) or (  # time signature
            (
                (
            ...
            ((current in all_diacritics) or
                # no more than 2 '~' for each note
                (current == operators['prolonger'] and not re.search('~[-=\'\"<>\[\{`]*~', previous)) or
                # inverted ornaments
                (current == operators['inverter'] and re.search('[^\[](\[|\{)$', previous)) or
                # mordent, trills, no double ornamentation
                (current == '[' and (re.search('[^\[]\[{1,5}$', previous) or not re.search('\[', previous)) and not re.search('\{', previous)) or
                # grupetto, no double ornamentation
                (current == '{' and not re.search('\[|\{', previous)) or
                # beams
                (current == '.' and not re.search('\.\.', previous)) or
                (current == operators[
                 'inverter'] and not re.search('`[-=\'\"<>\[\{~]*`', previous))  # stem inversion
             ) and (
                previous[0] in note_range
            )
        ) or (  # dynamics
            (current in dynamics_symbols or current == operators['inverter']) and (
                previous[0] in dynamics_symbols) and ((previous + current) in dynamics)
        ) or (  # gradual dynamics: crescendo, long form
            current == 'l' and previous == 'l'
        ) or (  # repeats
            (current == 'o' or current == operators[
             'inverter']) and previous[-1] == 'o'
        ) or (  # repeat references with indications
            current in ['i', 'I'] and previous in repeat_reference
        ) or (  # octavation
            (current == 'O' or current == operators[
             'inverter']) and previous == 'O'
        ) or (  # pedal mark 'up'
            ...
            current == punctuation['bold_double_barline'] and previous == punctuation[
                'bold_double_barline']
        ) or (  # bold
...
def parse(content, inputLanguage=None):
    ...
    tree = []
    # this variable is the last defined key signature and affects all
    current_key_signature = KeySignature(clefs['+'])
    ...
    for tokenIndex, token in enumerate(stack):
        ...
        if token == '\n':
            ...
            if (split_token and split_token.group(1) in clefs):
                ...
                tree.append(
                    KeySignature(clefs[split_token.group(1)], new_key_signature))
                current_key_signature = KeySignature(
                    clefs[split_token.group(1)], new_key_signature)
                continue
        ...
        # internally used to create beams between eighth notes
        elif token == '..':
            tree.append(ErrorSign())
            continue
            ...
            try:
                ...
                if globalPos in boldIndexSet:
                    ...
                    if note_pitch >= eighth_note_range:  # eighth notes
                        chord_notes.append(
                            Note(note_pitch, current_key_signature, length_exponent=1, denominator=16))
                    else:  # 16th notes
                        chord_notes.append(
                            Note(note_pitch, current_key_signature, length_exponent=0, denominator=16))
                else:
                    # Regular weight ~ 1/8 unity
                    if note_pitch >= eighth_note_range:  # quarter notes
                        chord_notes.append(
                            Note(note_pitch, current_key_signature, length_exponent=1))
                    else:  # eighth notes
                        chord_notes.append(
                            Note(note_pitch, current_key_signature, length_exponent=0))
                ...
                if symbol == operators['prolonger'] and len(chord_notes) > 0:
                    ...
                    beamed = True
                       ...
                       # for the case of inverted ornamentation
                       (symbol == operators['inverter'] and token[symbolIndex - 1] in ornaments_symbols) or
                       symbol == accent_mark or
                       symbol == tie
                       ) and len(chord_notes) > 0):
                    chord_notes[-1].add_diacritical_mark(symbol)
                # stem inversion
                elif symbol == operators['inverter'] and len(chord_notes) > 0:
                    chord_notes[-1].invertStem()
                elif symbol in simple_punctuation:  # beams
                    pass
                # pseudo-spaces
                elif symbol == punctuation['special_splitter']:
                    pass
                else:
        ...
        if len(chord_notes) > 0:
            ...
            raise SyntaxException([tokenIndex, stack, ''])
